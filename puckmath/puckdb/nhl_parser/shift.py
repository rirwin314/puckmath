...
class NHLShift(object):
    def __init__(self, soup):
        ...
        self.shift_start_elapsed = text_to_time(_elapsed_text)
        self.shift_start_remaining = text_to_time(_remain_text)
        _end_text = cols[3].text
        _elapsed_text = _end_text.split('/')[0].strip()
        _remain_text = _end_text.split('/')[1].strip()
        self.shift_end_elapsed = text_to_time(_elapsed_text)
        self.shift_end_remaining = text_to_time(_remain_text)
        self.duration = text_to_time(cols[4].text)
